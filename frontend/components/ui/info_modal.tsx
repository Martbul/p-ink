  ...
  const modal = (
    ...
    >
        ...
        <div className="relative z-10 p-8">
              ...
              <div>
                ...
                <h2
                  ...
                  style={{
                    ...
                    textShadow: `0 0 20px ${cfg.color}30`,
                  }}
                >
                  {title}
                </h2>
          ...
          {body && (
            <div className="font-mono text-sm leading-relaxed text-white/50 mb-4">
              {typeof body === "string" ? (
                <p>
                  <span style={{ color: cfg.color }}>{">"}</span> {body}
                </p>
              ) : (
                body
              )}
      ...
      <style>{`
        ...
        .animate-modal-in { animation: modal-in 0.25s cubic-bezier(.16,1,.3,1) both; }
      `}</style>
    </div>
  );
...
export function useModal() {
  ...
  const open = useCallback(
    (config: ModalConfig) => setState({ open: true, config }),
    [],
  );
...
export const ModalPresets = {
  ...
  pairError: (reason: string): ModalConfig => ({
    ...
    detail: (
      <p className="font-mono text-[10px] text-white/30 uppercase tracking-widest leading-relaxed">
        Check the MAC address on the back of your frame and try again. Make sure
  ...
  coupleActivated: (partnerName: string): ModalConfig => ({
    ...
    actions: [
      { label: "Enter matrix →", variant: "primary", onClick: () => {} },
    ],
  }),
