...
// ─── Clip paths (shared across all app surfaces) ──────────────────────────────
export const polyClip        = "polygon(20px 0%, 100% 0%, 100% calc(100% - 20px), calc(100% - 20px) 100%, 0% 100%, 0% 20px)";
export const polyClipReverse = "polygon(0 0, calc(100% - 20px) 0, 100% 20px, 100% 100%, 20px 100%, 0 calc(100% - 20px))";
export const polySmall       = "polygon(10px 0%, 100% 0%, 100% calc(100% - 10px), calc(100% - 10px) 100%, 0% 100%, 0% 10px)";
export const polyTiny        = "polygon(6px 0%, 100% 0%, 100% calc(100% - 6px), calc(100% - 6px) 100%, 0% 100%, 0% 6px)";
      ...
      <div className="flex items-center gap-2">
        ...
        {partnerUser && (
          ...
          >
            ...
            />
            <span className="font-mono text-[10px] uppercase tracking-widest" style={{ color: "rgba(177,34,229,0.8)" }}>
...
// ─── AppLayout ────────────────────────────────────────────────────────────────
// No sidebar. Just topbar + slide-over settings panel.
export function AppLayout({ children }: { children: React.ReactNode }) {
  const [settingsOpen, setSettingsOpen] = useState(false);

  return (
    <div className="min-h-screen flex flex-col" style={{ background: "#07070f", color: "#fff" }}>
      ...
      {/* Settings slide-over — rendered here so it's always available */}
      <SettingsPanel
        open={settingsOpen}
