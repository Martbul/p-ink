...
import { DeviceWithState } from "@/types/api";
...
export default function SettingsPage() {
  ...
  // FIXED — device from context always works (uses owner_id, not couple_id)
  const myDevice: DeviceWithState | undefined = device
    ? { device, frame_state: frameState }
    : coupleDevices.find((d) => d.device.owner_id === user?.id);
        ...
        <div className="max-w-2xl mx-auto px-6 relative z-10 animate-fade-in">
            ...
            {!couple ? (
              ...
              <div className="flex flex-col gap-4">
                <p className="font-mono text-[10px] text-yellow-400 uppercase tracking-widest animate-pulse flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-yellow-400 block rounded-full" />
              ...
              <>
                <CyberRow
                  label={partnerUser?.name ?? "REMOTE_NODE"}
                  sub={
                    "ID: " +
          ...
          <CyberPanel title="HARDWARE_TERMINALS">
            <p className="font-mono text-[10px] text-text-muted uppercase tracking-widest mb-6 border-l-2 border-neon-blue/40 pl-3 py-1 bg-neon-blue/5">
              &gt; Each partner has their own e-ink terminal. A couple has two
              devices total.
            </p>

            <CyberRow
              label="Server Timezone"
              sub="Determines 00:00 cycle reset for the couple"
            >
              <select
            ...
            <div className="mt-6 pt-4 border-t border-white/5">
              <p className="font-mono text-[10px] text-neon-blue uppercase tracking-widest mb-3 flex items-center gap-2">
                <span className="w-2 h-2 bg-neon-blue" />
                MY TERMINAL
              </p>
              {myDevice ? (
                <>
                  ...
                  >
                    <CyberBadge
                      text={
                        isOnline(myDevice.device.last_seen)
                          ? "ONLINE"
                          : "OFFLINE"
                      }
                      status={
                  ...
                  <CyberRow
                    ...
                    sub={"VER // " + (myDevice.device.firmware ?? "UNKNOWN")}
                  >
                ...
                <CyberRow
                  label="No Terminal Linked"
                  sub="Pair your e-ink frame from the dashboard."
                >
            ...
            {couple?.status === "active" && (
              <div className="mt-6 pt-4 border-t border-white/5">
                <p className="font-mono text-[10px] text-neon-purple uppercase tracking-widest mb-3 flex items-center gap-2">
                  ...
                  PARTNER TERMINAL [
                  {(partnerUser?.name ?? "REMOTE_NODE").toUpperCase()}]
                </p>
                {partnerDevice ? (
                  <>
                    <CyberRow
                      label="MAC_ADDRESS_ID"
                      sub={
                        partnerDevice.device.mac_address +
                        " // LAST_PING: " +
                    ...
                    >
                      <CyberBadge
                        text={
                          isOnline(partnerDevice.device.last_seen)
                            ? "ONLINE"
                            : "OFFLINE"
                        }
                        status={
                    ...
                    <CyberRow
                      label="Firmware Version"
                      sub={
                        "VER // " + (partnerDevice.device.firmware ?? "UNKNOWN")
                      }
                    >
                      <CyberBadge text="UP_TO_DATE" status="neutral" />
                    </CyberRow>
                  ...
                  <CyberRow
                    label="Partner has not paired their terminal yet"
                    sub="They can pair their device from their dashboard."
                  >
                    <CyberBadge text="UNPAIRED" status="offline" />
                  </CyberRow>
                )}
          ...
          <div className="mt-12 pt-8 border-t border-red-500/20">
            ...
            >
              ...
              >
                <CyberButton
                  onClick={() => confirm("Purge account?") && undefined}
                  variant="danger"
                >
                  Execute Purge
                </CyberButton>
