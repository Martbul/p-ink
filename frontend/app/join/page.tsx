...
function JoinPageInner() {
      ...
      .catch((err: unknown) => {
        if (!cancelled) {
          if (err instanceof ApiError) {
            setFetchError(
              (err as ApiError & { status: number }).status === 410
  ...
  useEffect(() => {
    ...
    if (couple) {
      setJoinError(
        "You're already in a pending couple. You cannot join another.",
      );
      return;
    }
    ...
    >
      ...
      <div className="w-full max-w-md bg-white rounded-3xl p-10 shadow-xl border border-[rgba(232,197,176,0.3)]">
        ...
        >
          ...
          The link expires{" "}
          {info
            ? new Date(info.expires_at).toLocaleDateString("en-US", {
                month: "long",
                day: "numeric",
              })
            : "soon"}
